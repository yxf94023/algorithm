 ...
 *\param[in] pg 图的句柄
 */
static void exec_dfs(const adj_graph_st *pg)
{
	...
	for (unsigned int i = 0; i < pg->ver_cnt; ++i){
		...
		express_flag[i] = 1;
		stack_push(&stk, &pg->ver_list[i]);	///< 进栈
		++cnt;
		
		while (!stack_empty(&stk) && stack_get_top(&stk, (void**)(&p_tmpver))){
			...
			while (NULL != p_tmparc){
				...
				if (express_flag[p_tmparc->index] == 0){
					...
					stack_push(&stk, &pg->ver_list[p_tmparc->index]);
					break;
				}
			...
			if (NULL == p_tmparc){/// 当前 顶点连上的 结点都已经被访问过
				
				stack_pop(&stk, (void**)(&p_tmpver));	///< 出栈
			}
		}		
	}
