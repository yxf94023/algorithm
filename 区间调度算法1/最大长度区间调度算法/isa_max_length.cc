 ...
 *\detail 最长区间调度算法，在所有的区间中选择不重叠的区间的最大长度
 *\date 2017/03/11
 ******************************************************************************/
...
static void isa_print_inputs(void){
	...
	{
		printf("(%02u): %u - %u\n", i, s_isa_array[i].uint_beg, s_isa_array[i].uint_end);
	}	
}
...
static void isa_do_work(void){
	...
	s_isa_res[0].max_length = s_isa_array[0].uint_end - s_isa_array[0].uint_beg;
	...
	for (i = 1; i < s_isa_cnt; ++i){
		...
		if (k > 0){/* 表示 存在不重叠的情况 */
			...
			max = s_isa_array[i].uint_end - s_isa_array[i].uint_beg;
			tmp_set.max_length = max;
			tmp_set.uint_cnt = 1;
